...
function querytempproject(obj,templateid){
	var pro = $('#addprojectlist').find('tr[name="addproject_'+templateid+'"]');
	if(pro.length == 0){
		...
		param.templatename = $(obj).text();
		queryprojecttemplate(param);
...
/*---------------------------------------------------------------------------------------------------------------------------------------------*/
function queryprojecttemplate(param){
	requestPost("/userbackstage/getOrganizeTemplateProjectListInfo",param,function(resultMap){
		if(resultMap != null && resultMap != ''){
			var prolist = resultMap.projectlist;
			if(prolist != null && prolist != ''){
				var html = '<tr name="addproject_'+param.templateid+'">'+
								'<td colspan="2" name="add_pro_name" style="text-align:left; font-weight:bold; font-size:15px;" status="1" templateid="'+param.templateid+'">'+param.templatename+'</td>'+
				            '</tr>';
				$.each(prolist,function(i,map){
					html += '<tr name="addproject_'+map.templateid+'">'+
								'<td name="add_pro_name" status="0" templateid="'+param.templateid+'">'+map.projectname+'</td>'+
				                '<td class="t_r"><a href="javascript:void(0)" class="blue m_r30" onclick="showTemplateProject(this)">修改</a><a href="javascript:void(0)" onclick="deleteTemplateprojectInfo(this,\''+param.templateid+'\')" class="del">删除</a></td>'+
				            '</tr>';
				});
...
function getTemplateList(){
	requestPost("/userbackstage/getEvaluateTemplate",pageParam,function(resultMap){
	...
	if(list.length > 0 && list != null){
		...
		$.each(list,function(i,map){
			var htm = '<tr id="template_'+map.templateid+'">'+
		            	'<td id="templatename_'+map.templateid+'">'+map.organizename+"-"+map.templatename+'</td>'+
		                '<td><a href="javascript:void(0)" onclick="queryTemplateProject(\''+map.templateid+'\',\''+map.templatename+'\',\''+map.organizeid+'\');" class="link">修改</a>'+
		            	'<a href="javascript:void(0)" onclick="deleteInspectTemplate(\''+map.templateid+'\',\''+map.templatename+'\')" class="del">删除</a></td>'+
		            '</tr>';
			$('#template_list').append(htm);
		});
	}
}
...
function deleteInspectTemplate(templateid,templatename){
	swal({
		...
		param.templateid = templateid;
		param.delflag = 1;
		updateEvaluateTemplate(param,function(resultMap){
			if(resultMap.status == 0 || resultMap.status == '0'){
...
function updateEvaluateTemplate(param,callback){
	requestPost("/userbackstage/updateEvaluateTemplate",param,function(resultMap){
		callback(resultMap);
	});
}
...
function addProjectAndTemplate(){
	...
	if(name != ""){
		...
		param.organizeid = region_organizeid;
		param.templatename = name;
		requestPost("/userbackstage/getEvaluateTemplateNameIsExists",param,function(data){
			if(data == "exists"){
				swal({
				    title: "提示",
				    text: "该组织机构下，区域模板名称重复了！",
				    type: "warning",
				    showCancelButton: false,
				    confirmButtonColor: "#ff7922",
				    confirmButtonText: "确定",
				...
				$.each(project,function(k,pro){
					var obj = new Object();
					obj.projectname = $(pro).text();
					obj.status = parseInt($(pro).attr("status"));
				...
				param.type = type;
				queryTemplateProject_div();
				requestPost("/userbackstage/insertEvaluateTemplateInfo",param,function(resultMap){
					if(resultMap.status == 0 || resultMap.status == '0'){
						getTemplateList();
					}
				});
	...
	if(name != ""){
		...
		requestPost("/userbackstage/getEvaluateTemplateNameIsExists",param,function(data){
			if(data == "exists"){
				...
				queryTemplateProject_div();
				requestPost("/userbackstage/updateEvaluateTemplateProject",param,function(resultMap){
					if(resultMap.status == 0 || resultMap.status == '0'){
						swal("","修改成功","success");
						getTemplateList();
					}
...
function queryTemplateProject(templateid,templatename,organizeid){
	...
	var param = new Object();
	param.templateid = templateid;
	requestPost("/userbackstage/getEvaluateTemplateDetail",param,function(resultMap){
		if(resultMap != null && resultMap != ""){
			var projectlist = resultMap.detailmap.projectlist;
			var rangelist = resultMap.detailmap.rangelist;
				...
				$.each(projectlist,function(i,map){
					...
					if(status == 1 || status == "1"){
						html += '<tr name="addproject_'+map.resoucetemplateid+'">'+
									'<td colspan="2" name="add_pro_name" style="text-align:left; font-weight:bold; font-size:15px;" status="1" templateid="'+map.resoucetemplateid+'">'+map.projectname+'</td>'+
					            '</tr>';
					}else{
						html += '<tr name="addproject_'+map.resoucetemplateid+'">'+
	...
	if(templateid != "" && templateid != undefined){
		...
		requestPost("/userbackstage/insertOrganizeTemplateProjectInfo",param,function(data){
			if(data != "error"){
				...
				swal({   
					title: "",   
					text: "<font style=\"color:#ff7922; font-size:28px; margin-left:20px;\">新增成功！</font>",
					type:"success",
					timer: 1000,
					html:true,

var fangwen_url = "/app/getAllEvaluateList";
...
function queryNoExamineInfo(obj){
	requestPost(fangwen_url,obj,"allExamine",false,function(resultMap){
		if(resultMap != undefined && resultMap != null){
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(i,map){
			...
			var onclick_url = 'jobstar_detail_by_userid.html?evaluateid='+map.evaluateid+"&userid="+userid+'&forwarduserid='+map.forwarduserid;
			...
			if($('#'+map.evaluateid).length <= 0){
				htm = '<li id="'+map.evaluateid+'" onclick="intoDetail(\''+onclick_url+'\',\''+map.evaluateid+'\',\''+map.isread+'\',6)">'+

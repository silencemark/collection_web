...
function showcommentnoneanddetail(userid){
	if(userid.indexOf(userInfo.userid) >= 0){
		$("#pinglun_none").show();
		$("#pinglun_detail").show();
	}
}
...
function showDetail(evaluateInfo){
	if(evaluateInfo != null && evaluateInfo != ''){
		...
		showcommentnoneanddetail(evaluateInfo.createid);
		showcommentnoneanddetail(evaluateInfo.examineuserid);
		$.each(evaluateInfo.ccuserlist,function(i,item){
			showcommentnoneanddetail(item.userid);
		});
		$.each(evaluateInfo.forwardnamelist,function(i,item2){
			showcommentnoneanddetail(item2.forwarduserids);
		});

var fangwen_url = "/app/getExamineRequestList";
var userid = "";
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			var requestlist = item.requestlist;
			...
			$.each(requestlist,function(i,map){
				var isread = '';
				//判断是否阅读
				...
				var htm = '<li class="li_del" style="height:127px;" onclick="intoDetail(\''+onclick_url+'\',\''+map.requestid+'\',\''+map.isread+'\',16)">'+
							'<div class="li_box" id="'+map.forwarduserid+'_libox">'+
					        	'<div class="xx_01"><span '+isread+'>'+map.createname+'的请示</span>'+resulthtml+'</div>'+
				        	...
				        	'<div class="clear"></div>'+
				            '<div class="xx_02" style="padding:0px 15px;">'+jinji+'<i>'+map.createtime+'</i></div>'+
				        '</li>';
				//显示
				$('#'+createdate+'_ul').append(htm);
			});
			//添加数据条数
			$('#'+createdate+'_i').text(requestlist.length);

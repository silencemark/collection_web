	...
	$.ajax({
		...
		success:function(data){
			if(data.status == 0 || data.status == '0'){
				...
				if(overnum > 0){
					$('.span_nav2 span[class="mid active"]').prepend(redPoint);
				}
				if(examinenum > 0){
	...
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			...
			$.each(tasklist,function(i,map){
				...
				var assi = "";
				var assistlist = map.assistlist;
				if(assistlist != undefined && assistlist != null){
					$.each(assistlist,function(k,item){
						if(item != null){
							assi += item.realname+"，";
						}
					});
				...
				var onclick_url = 'task_detail.html?taskid='+map.taskid+"&userid="+userid+'&forwarduserid='+map.forwarduserid;
				htm = '<li class="li_del" style="height:127px;" onclick="intoDetail(\''+onclick_url+'\',\''+map.taskid+'\','+map.isread+',19)">'+
							'<div class="li_box" id="'+map.forwarduserid+'_libox">'+
								'<div class="xx_01"><span '+isread+'>'+map.createname+'发布的任务</span><em class="gray">已过期</em></div>'+

var fangwen_url = "/app/getExamineLeavelist";
var userid = "";
	...
	if(list.length > 0 && list != null){
		$.each(list,function(k,item){
			...
			$.each(leavelist,function(i,map){
				...
				var htm = '<li class="li_del" style="height:127px;" onclick="intoDetail(\''+onclick_url+'\',\''+map.leaveid+'\','+map.isread+',18)">'+
							'<div class="li_box" id="'+map.forwarduserid+'_libox">'+
					        	'<div class="xx_01"><span '+isread+'>'+map.createname+'的请假单</span>'+resulthtml+'</div>'+
				        	...
				        	'<div class="clear"></div>'+
				            '<div class="xx_02" style="padding:0px 15px;"><span class="qj">'+map.leavetypename+'</span><span><em>'+map.daynum+'</em>天<em>'+map.hournum+'</em>时</span></span><i>'+map.createtime+'</i></div>'+
				        '</li>';
				//显示
				$('#'+createdate+'_ul').append(htm);
			});
			//添加数据条数
			$('#'+createdate+'_i').text(leavelist.length);

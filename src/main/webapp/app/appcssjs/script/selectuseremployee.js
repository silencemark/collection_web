...
function changerealnamenew(obj,userid,realname){
	$('a[class=radio_ed]').attr("class","radio");
		...
		$.ajax({
			...
			success:function(data){
				if(data.userlist.length > 0){
					...
					if(data.userlist.length > 0){
						for(var j=0;j<data.userlist.length ;j++){
							temp+="<div name=\"people\"><i class=\"ico_person\" style=\"padding:0px 15px 0px 30px;background-position:15px 50%;\">"+data.userlist[j].realname+"-"+data.userlist[j].organizename+"</i><a class=\"radio\" onclick=\"changerealnamenew(this,'"+data.userlist[j].userid+"','"+data.userlist[j].realname+"')\">选择</a><div class=\"clear\"></div></div>";
						}
					}
	        		temp+="</li></ul>";
	        		$("#organizetext").nextAll().remove();

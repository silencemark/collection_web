			...
			if(data.organizelist.length > 0){
				...
				for(var i=0;i<data.organizelist.length ;i++){
					if(data.organizelist[i].type!=3){
						...
						temp+=" style=\"padding:0px 15px 0px "+(num*15+30)+"px;background-position:"+(num*15+15)+"px 50%;\" onclick=\"getorganizelist("+(num+1)+",'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\"><i class=\""+changeorganizetype(data.organizelist[i].type)+"\">"+data.organizelist[i].organizename+"</i><div class=\"clear\"></div></span>";
					}
					if(data.organizelist[i].type==3){
						if(data.organizelist[i].isnopower !=1){
							temp+="<div name=\"people\"  onclick=\"changerealname(this,'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"','"+data.organizelist[i].datacode+"','"+data.organizelist[i].qrcode+"')\"><i class=\""+changeorganizetype(data.organizelist[i].type)+"\" style=\"padding:0px 15px 0px "+(num*15+35)+"px;background-position:"+(num*15+15)+"px 50%;\">"+data.organizelist[i].organizename+"</i><a class=\"radio\">选择</a><div class=\"clear\"></div></div>";
						}
					}
				}
        		temp+="</li></ul>";
...
function changerealname(obj,organizeid,organizename,datacode,qrcode){
		...
		$.ajax({
			...
			success:function(data){
				if(data.organizelist.length > 0){
					...
					for(var i=0;i<data.organizelist.length ;i++){
						if(data.organizelist[i].type==3){
							temp+="<div name=\"people\" onclick=\"changerealname(this,'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"','"+data.organizelist[i].datacode+"','"+data.organizelist[i].qrcode+"')\"><i class=\""+changeorganizetype(data.organizelist[i].type)+"\" style=\"padding:0px 15px 0px 30px;background-position:15px 50%;\">"+data.organizelist[i].organizename+"</i><a class=\"radio\">选择</a><div class=\"clear\"></div></div>";
						}
					}
					temp+="</li></ul>";
					$("#organizetext").nextAll().remove();

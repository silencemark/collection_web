...
"<div style=\"display: none;width:100%;height:100%\" id=\"useriframe\">"+
		"<input type=\"hidden\" id=\"userid\"/>"+
"<input type=\"hidden\" id=\"realname\"/>"+
"<div class=\"head_box\">"+
"<a class=\"ico_back\" onclick=\"$('#useriframe').hide();$('#htmlbody').show();\">返回</a>"+
		...
		success:function(data){
			if(data.organizelist.length > 0 || data.userlist.length >0){
				...
				for(var i=0;i<data.organizelist.length ;i++){
					...
					temp+="id=\"organize_"+data.organizelist[i].datacode+"\" style=\"padding:0px 15px 0px "+(num*15+30)+"px;background-position:"+(num*15+15)+"px 50%;\"  onclick=\"getuserlist("+(num+1)+",'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" ><i class=\""+changeorganizetype(data.organizelist[i].type)+"\">"+data.organizelist[i].organizename+"</i><div class=\"clear\"></div></span>";
				}
				
				if(data.userlist.length > 0){
					for(var j=0;j<data.userlist.length ;j++){
						if(data.userlist[j].userid != userInfo.userid){
							temp+="<div name=\"people\" onclick=\"changeusername(this,'"+data.userlist[j].userid+"','"+data.userlist[j].realname+"')\"><i class=\"ico_person\" style=\"padding:0px 15px 0px "+(num*15+35)+"px;background-position:"+(num*15+15)+"px 50%;\">"+data.userlist[j].realname+"</i><a class=\"radio\" >选择</a><div class=\"clear\"></div></div>";
						}
					}
				}
        		temp+="</li></ul>";
...
function changeusername(obj,userid,realname){
		...
		$.ajax({
			...
			success:function(data){
				if(data.userlist.length > 0){
					...
					if(data.userlist.length > 0){
						for(var j=0;j<data.userlist.length ;j++){
							temp+="<div name=\"people\" onclick=\"changeusername(this,'"+data.userlist[j].userid+"','"+data.userlist[j].realname+"')\"><i class=\"ico_person\" style=\"padding:0px 15px 0px 30px;background-position:15px 50%;\">"+data.userlist[j].realname+"-"+data.userlist[j].organizename+"</i><a class=\"radio\">选择</a><div class=\"clear\"></div></div>";
						}
					}
	        		temp+="</li></ul>";
	        		$("#usertext").nextAll().remove();

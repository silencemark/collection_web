...
var htmlOrganize = "<link type=\"text/css\" rel=\"stylesheet\" href=\"../appcssjs/style/structure.css\">"+
					"<div style=\"display: none;width:100%;height:100%\" id=\"morechange_organizeiframe\">"+
					'<div class="head_box">'+
					...
					'<div class="stru_list">'+
					    ...
					    '<div class="search_list" id="morechange_orgainizediv">'+
					        ...
					        '<div class="clear"></div>'+
					    '</div>'+
					    '<div class="title" id="morechange_organizetext" onclick="onloadDataorg()">上海餐饮集团有限公司</div>'+
					'</div></div>';
				...
				if(data.userlist.length > 0){
					for(var j=0;j<data.userlist.length ;j++){
						if(data.userlist[j].userid != userInfo.userid){
							temp+="<span class=\"leve_a_s\" style=\"padding:0px 15px 0px "+(num*15+20)+"px;background-position:"+(num*15+15)+"px 50%;\"><a class=\"check\"  onclick=\"chooseuser('"+data.userlist[j].userid+"','"+data.userlist[j].realname+"',this)\" name=\"choose\">选择</a><i class=\"ico_person\">"+data.userlist[j].realname+"</i><div class=\"clear\"></div></span>";
						}
					}
				}
        		temp+="</li></ul>";
		...
		$.ajax({
			...
			success:function(data){
				if(data.organizelist.length > 0 || data.userlist.length >0){
					...
					temp+="<li class=\"leve_a\" >";
					for(var i=0;i<data.organizelist.length ;i++){
						temp+="<span class=\"leve_a_s\" style=\"padding:0px 15px 0px 15px;background-position:15px 50%;\"><em class=\"em_down\"  onclick=\"morechange_getorganizelist(1,'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" >显示</em><i class=\""+changeorganizetype(data.organizelist[i].type)+"\" onclick=\"morechange_getorganizelist(1,'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" >"+data.organizelist[i].organizename+"</i><div class=\"clear\"></div></span>";
					}
					if(data.userlist.length > 0){
						for(var j=0;j<data.userlist.length ;j++){

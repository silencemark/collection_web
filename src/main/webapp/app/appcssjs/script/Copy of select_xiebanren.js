...
var htmlOrganize = "<div style=\"display: none;width:100%;height:100%\" id=\"morechange_organizeiframe\">"+
					...
					'<div class="stru_list">'+
					    ...
					    '<div class="search_list" id="morechange_orgainizediv">'+
					        ...
					        '<div class="clear"></div>'+
					    '</div>'+
					    '<div class="title" id="morechange_organizetext" onclick="onloadData()">上海餐饮集团有限公司</div>'+
					'</div></div>';
		...
		success:function(data){
			if(data.organizelist.length > 0 || data.userlist.length >0){
				...
				for(var i=0;i<data.organizelist.length ;i++){
					temp+="<span class=\"leve_a_s\" style=\"padding:0px 15px 0px "+(num*30+15)+"px;background-position:"+(num*30+15)+"px 50%;\"><i onclick=\"morechange_getorganizelist("+(num+1)+",'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" >"+data.organizelist[i].organizename+"</i><em class=\"em_down\"  onclick=\"morechange_getorganizelist("+(num+1)+",'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" >显示</em><div class=\"clear\"></div></span>";
				}
				if(data.userlist.length > 0){
					for(var j=0;j<data.userlist.length ;j++){
						if(data.userlist[j].userid != userInfo.userid){
							temp+="<span class=\"leve_a_s\" style=\"padding:0px 15px 0px "+(num*30+15)+"px;background-position:"+(num*30+15)+"px 50%;\"><a class=\"check\"  onclick=\"chooseuser('"+data.userlist[j].userid+"','"+data.userlist[j].realname+"',this)\" name=\"choose\">选择</a><i>"+data.userlist[j].realname+"</i><div class=\"clear\"></div></span>";
						}
					}
				}
        		temp+="</li></ul>";
		...
		$.ajax({
			...
			success:function(data){
				if(data.organizelist.length > 0 || data.userlist.length >0){
					...
					temp+="<li class=\"leve_a\" >";
					for(var i=0;i<data.organizelist.length ;i++){
						temp+="<span class=\"leve_a_s\" style=\"padding:0px 15px 0px 15px;background-position:15px 50%;\"><a class=\"check\" onclick=\"chooseorganize('"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" name=\"choose\">选择</a><i  onclick=\"morechange_getorganizelist(1,'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" >"+data.organizelist[i].organizename+"</i><em class=\"em_down\"  onclick=\"morechange_getorganizelist(1,'"+data.organizelist[i].organizeid+"','"+data.organizelist[i].organizename+"',this)\" >显示</em><div class=\"clear\"></div></span>";
					}
					if(data.userlist.length > 0){
						for(var j=0;j<data.userlist.length ;j++){

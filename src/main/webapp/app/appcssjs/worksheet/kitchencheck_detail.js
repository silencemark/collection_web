...
function beformealDetail(){
	...
	obj.userid = userid;
	requestPost('/app/getKitchenInfo',obj,'detail_'+inspectid,true,function(resultMap){
		if(resultMap != undefined && resultMap != null){
...
function sendBeforeMeal(chun){
	if(shenhe_examineid == userid && shenhe_examinestatus == 0){
		...
		param.userid = userid;
		$.ajax({
			url:projectpath+"/app/examineKitchen",
			type:"post",
...
function logical(){
	...
	swal({
		...
		userForword(userInfo.companyid,inspectid,$('#examineuserid').val(),userInfo.userid,12);

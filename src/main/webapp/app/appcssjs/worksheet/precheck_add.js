				...
				$.each(projectlist,function(i,map){
					...
					if(status == "1" || status == 1){
						var project = '<li class="n_bor" id="'+map.projectid+'_project_li">'+
									    	...
									    	'<b><input type="hidden" name="input_star" id="'+map.projectid+'_input" projectid="'+map.projectid+'" projectname="'+map.projectname+'" status="1" sumNum="0"/></b>'+
									        '<div class="clear"></div>'+
									    '</li>';
	...
	if(list.length > 0 && list != null){
		...
		$.each(list,function(i,item){
			if(item != null){
				var cls = "";
				if(i==0){
					organizeid = list[i].organizeid;
					$('#organizename').val(item.organizename);
					cls = 'class="active"';
				}
				if(item.organizeid == userInfo.organizeid){
					changeorganizestore(item.datacode);
				}
				html += '<li '+cls+' name="organize_li" organizeid="'+item.organizeid+'" organizename="'+item.organizename+'" datacode="'+item.datacode+'">'+item.organizename+'</li>';
			}
		});
...
function addEvaluate(){
	...
	$.ajax({
		url:projectpath+"/app/insertBeforeMeal",
		type:"post",

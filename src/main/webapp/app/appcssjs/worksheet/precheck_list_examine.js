var fangwen_url = "/app/getExamineBeforeMealList";
var userid = "";
var nodata = '<div class="list_none"><span><i class="yellow">Hi,我是大狮！</i><br>列表空空，还没有餐前检查呢~</span><b><img src="'+notDataImageUrl_nopower+'"></b></div>';
var obj = new Object();
...
$(function(){
	...
	JianKangCache.getGlobalData('userinfo',function(data){
		...
		if(typeof(data.powerMap.power4001210) != "undefined"){
			$('#linkadd').show();
			nodata = '<div class="list_none"><span><i class="yellow">Hello,我是大狮！</i><br>还没有餐前检查，赶紧添加一条吧~</span><b><img src="'+notDataImageUrl_havepower+'"></b></div>';
		}
...
function getNotIsRead(param){
	$.ajax({
		url:projectpath+"/app/getBeforeMealAllNotRead",
		type:"post",
		data:param,
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			var beforemeallist = item.beforemeallist;
			...
			$.each(beforemeallist,function(i,map){
				var isread = '';
				//判断是否阅读
				if(map.isread == 0 || map.isread == '0'){
					isread = 'class="new"';
				}
				var onclick_url = 'precheck_detail.html?inspectid='+map.inspectid+"&userid="+userid+'&forwarduserid='+map.forwarduserid;
				//添加详细列表
				var htm = '<li class="li_del" onclick="intoDetail(\''+onclick_url+'\',\''+map.inspectid+'\',\''+map.isread+'\',11)">'+
							'<div class="li_box" id="'+map.forwarduserid+'_libox">'+
				        	'<div class="xx_01"><span '+isread+'><span class="name">'+map.createname+'的餐前检查</span></span><em class="green">已处理</em></div>'+
			...
			//添加数据条数
			$('#'+createdate+'_i').text(beforemeallist.length);

var fangwen_url = "/app/getExamineLeaveShopList";
var userid = "";
var nodata = '<div class="list_none"><span><i class="yellow">Hi,我是大狮！</i><br>列表空空，还没有离店报告单呢~</span><b><img src="'+notDataImageUrl_nopower+'"></b></div>';
var obj = new Object();
...
$(function(){
	...
	JianKangCache.getGlobalData('userinfo',function(data){
		...
		if(typeof(data.powerMap.power4001310) != "undefined"){
			$('#linkadd').show();
			nodata = '<div class="list_none"><span><i class="yellow">Hello,我是大狮！</i><br>还没有离店报告单，赶紧添加一条吧~</span><b><img src="'+notDataImageUrl_havepower+'"></b></div>';
		}
...
function getNotIsRead(param){
	$.ajax({
		url:projectpath+"/app/getLeaveShopAllNotRead",
		type:"post",
		data:param,
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			var leaveshoplist = item.leaveshoplist;
			...
			$.each(leaveshoplist,function(i,map){
				var isread = '';
				//判断是否阅读
				if(map.isread == 0 || map.isread == '0'){
					isread = 'class="new"';
				}
				var onclick_url = 'leave_detail.html?inspectid='+map.inspectid+"&userid="+userid+'&forwarduserid='+map.forwarduserid;
				//添加详细列表
				var htm = '<li class="li_del" onclick="intoDetail(\''+onclick_url+'\',\''+map.inspectid+'\',\''+map.isread+'\',10)">'+
			...
			//添加数据条数
			$('#'+createdate+'_i').text(leaveshoplist.length);

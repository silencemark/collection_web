var fangwen_url = "/app/getExmaineDishesListInfo";
var userid = "";
...
function queryExamineInfo(obj){
	requestPost(fangwen_url,obj,"examine",false,function(resultMap){
		...
		}else{$(".list_none").remove();$("body").append(nodata);}
	});
}
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			var disheslist = item.disheslist;
			...
			$.each(disheslist,function(i,map){
				var isread = '';
				//判断是否阅读
				...
				var htm = '<li class="li_del" onclick="intoDetail(\''+onclick_url+'\',\''+map.dishesid+'\',\''+map.isread+'\',14)">'+
							'<div class="li_box" id="'+map.forwarduserid+'_libox">'+
				        	'<div class="xx_01"><span '+isread+'><span class="name">'+map.dishesname+'</span></span><em class="green">已处理</em></div>'+
				            '<div class="xx_02"><span class="lb">'+map.dishestype+'</span><span class="cb">成本 <em>￥'+map.costprice+'</em></span><span class="sj">售价 <em>￥'+map.price+'</em></span></div>'+
			...
			//添加数据条数
			$('#'+createdate+'_i').text(disheslist.length);

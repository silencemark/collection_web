var fangwen_url = "/app/getExamineKitchenList";
var userid = "";
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			var kitchenlist = item.kitchenlist;
			...
			$.each(kitchenlist,function(i,map){
				var isread = '';
				//判断是否阅读
				...
				//添加详细列表
				var htm = '<li class="li_del" onclick="intoDetail(\''+onclick_url+'\',\''+map.inspectid+'\',\''+map.isread+'\',12)">'+
							'<div class="li_box" id="'+map.forwarduserid+'_libox">'+
				        	'<div class="xx_01"><span '+isread+'><span class="name">'+map.createname+'的厨房检查</span></span><em class="green">已处理</em></div>'+
			...
			//添加数据条数
			$('#'+createdate+'_i').text(kitchenlist.length);

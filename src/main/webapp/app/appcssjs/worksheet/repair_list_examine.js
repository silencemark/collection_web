var fangwen_url = "/app/getExamineRepairListInfo";
var userid = "";
...
function showData(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(k,item){
			var repairlist = item.repairlist;
			...
			$.each(repairlist,function(i,map){
				var isread = '';
				//判断是否阅读
				...
				//添加详细列表
				var htm = '<li class="li_del" onclick="intoDetail(\''+onclick_url+'\',\''+map.repairid+'\',\''+map.isread+'\',13)">'+
							'<div class="li_box" id="'+map.id+'_libox">'+
				        	'<div class="xx_01"><span '+isread+'><span class="name">'+map.createname+'</span></span><em class="green">已处理</em></div>'+
			...
			//添加数据条数
			$('#'+createdate+'_i').text(repairlist.length);

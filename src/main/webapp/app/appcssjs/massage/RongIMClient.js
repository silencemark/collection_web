/**@overview 融云 RongCloud Web SDK API 开发文档*/
/** @global */
/**
 * 消息通道强制设置为长链接方式，默认为web
 * socket->xhr-polling层层降级方式.设置方式为window.WEB_XHR_POLLING=true;
 * 
 * WebSDK2.0 默认支持自动降级，也可以自动设置连接方式。
 * 
 * @example //在引入RongIMLib.js之前加入如下代码： <script>window["WEB_XHR_POLLING"] = true</script>
 */
...
function RongIMClient() {
  /**
	 * schemeType 选择连接方式 SSL需要设置schemeType为ConnectionChannel.HTTPS HTTP或WS需要设置
	 * schemeType为ConnectionChannel.HTTP(默认) RongIMLib.RongIMClient.schemeType =
	 * RongIMLib.ConnectionChannel.HTTP。
	 */
...
/**
 * 获取 RongIMClient 实例。 需在执行 init 方法初始化 SDK 后再获取，否则返回 null 值。
 * 
 * @example var rongClient = RongIMClient.getInstance();
 */
 ...
 * 
 * @param {string}
 *            appKey 开发者后台申请的 AppKey，用来标识应用
 * @param {object}
 *            dataAccessProvider 必须是DataAccessProvider的实例
 */
 ...
 * @param {object}
 *            callback 连接回调，返回连接的成功或者失败状态。
 * @example RongIMClient.connect('TOKEN', { onSuccess: function(userId) {
 *          console.log("Login successfully." + userId); }, onTokenIncorrect:
 *          function() { console.log('token无效'); }, onError:function(errorCode){
 *          var info = ''; switch (errorCode) { case
 *          RongIMLib.ErrorCode.TIMEOUT: info = '超时'; break; case
 *          RongIMLib.ErrorCode.UNKNOWN_ERROR: info = '未知错误'; break; case
 *          RongIMLib.ErrorCode.UNACCEPTABLE_PaROTOCOL_VERSION: info =
 *          '不可接受的协议版本'; break; case RongIMLib.ErrorCode.IDENTIFIER_REJECTED:
 *          info = 'appkey不正确'; break; case
 *          RongIMLib.ErrorCode.SERVER_UNAVAILABLE: info = '服务器不可用'; break; }
 ...
 * 
 * @param {object}
 *            callback 重连回调函数，与connect的回调函数一致
 * @example RongIMClient.reconnect({ onSuccess:function(){ //重连成功 },
 *          onError:function(){ //重连失败 } });
 */
RongIMClient.reconnect = function(callback) {
  RongIMClient.bridge.reconnect(callback);
};
/**
 * 注册消息类型，用于注册用户自定义的消息。 内建的消息类型已经注册过，不需要再次注册。
 * 自定义消息声明需放在执行顺序最高的位置（在RongIMClient.init(appkey)之后即可）
 * 
 * @param {string}
 *            messageType 消息类型（与自定义消息类名一致）。
 * @param {string}
 *            objectName 内置消息名称。
 * @param {MessageTag}
 *            messageTag 设置是否保存是否计数
 * @param {string|array}
 *            messageContent
 *            此参数有一参二用的功能，自定义消息结构及解析方式请传入自定义消息的实例，使用融云内置消息结构及解析方式，直接传入属性数组即可，例如：["name","age"]。
 * @example //自定义消息支持方式 //(1) 自定义消息结构及解析方式。<br/> //(2) SDK 内置消息结构及解析方式。<br/>
 *          //自定消息类型参数说明
 *          //`RongIMClient.registerMessageType('messageType','objectName','messageTag','message|[]')`<br/>
 *          //(1) `messageType`:消息类型（与自定义消息类名一致）。<br/> //(2)
 *          `objectName`:消息内置名称。<br/> //(3) `messageTag`:设置是否保存是否计数。<br/>
 *          //(4)
 *          `message|[]`:此参数有一参二用的功能，自定义消息结构及解析方式请传入自定义消息的实例，使用融云内置消息结构及解析方式，直接传入属性数组即可["name","age"]。<br/>
 *          //自定消息注意事项 //(1) 自定义消息结构: 任意形式，在 decode 中解析可即可。<br/> //(2) SDK
 *          内置消息结构: JSON 类型的字符串 `JSON.stringify({name:"zhangsan",age:12})`<br/>
 *          //(3) 自定义消息注册位置: RongIMClient.init("appkey") 之后的第一行位置注册即可。<br/>
 *          //注册自定义消息示例(消息结构自定义) //自定义消息结构，首先要定义消息类，其次是注册，EmptyMessage为自定义消息名称。<br/>
 *          //以下代码为示例代码，实际应用中可能有不规范之处，敬请谅解，自定义消息类型，格式必须与此一样，可修改的内容有：<br/>
 *          //1、消息类名称可修改为
 *          RongIMClient.RegisterMessage.***Message=function(message){<br/> //
 *          dosomething....<br/> //}。<br/> //2、messageName的值必须与消息类名称一致。<br/>
 *          //3、encode方法的实现可以自定义（编译消息，发送消息时用到）。<br/>
 *          //4、decode方法的实现可以自定义（解析消息，接受消息时用到）。<br/> //1、创建消息
 *          RongIMClient.RegisterMessage.PersonMessage = function(message){
 *          this.messageName = "PersonMessage"; this.encode = function(){ return
 *          "name="+message.name+";age="+message.age; } this.decode =
 *          function(message){ if (typeof message != "string") { return message; }
 *          var strArrs = message.split(";"),msg = new
 *          RongIMClient.RegisterMessage.PersonMessage(); for(var p in strArrs){
 *          msg[strArrs[p].split("=")[0]]=strArrs[p].split("=")[1]; } return
 *          msg; } }
 *          RongIMClient.registerMessageType('PersonMessage','s:empty',new
 *          RongIMLib.MessageTag(true,true),new
 *          RongIMClient.RegisterMessage.PersonMessage); //发送消息 var msg = new
 *          RongIMClient.RegisterMessage.PersonMessage({name:"zhang",age:12});
 *          RongIMClient.getInstance().sendMessage('convertype','targetId', msg,
 *          null, { onSuccess: function (data) { }, onError: function
 *          (errorCode) { } }); //注册自定义消息示例(内置消息结构) //创建消息
 *          RongIMClient.registerMessageType('s:empty','PersonMessage',new
 *          RongIMLib.MessageTag(true,true),["name","age"]); //发送消息 var msg =
 *          new
 *          RongIMClient.RegisterMessage.PersonMessage({name:"zhang",age:12});
 *          RongIMClient.getInstance().sendMessage('convertype','targetId', msg,
 *          null, { onSuccess: function (data) { }, onError: function
 ...
 * @param {object}
 *            listener 连接状态变化的监听器。
 * @example RongIMClient.setConnectionStatusListener({
 ...
 * @param {object}
 *            listener 接收消息的监听器
 * @example RongIMClient.getInstance().setOnReceiveMessageListener({
 *          onReceived:function(message){ //message 为具体的消息对象 } });
 */
 ...
 * 断开连接。
 * 
 * @example RongIMClient.getInstance().disconnect();
 */
 ...
 * 获取当前连接的状态。
 * 
 * @example var connectStatus =
 *          RongIMClient.getInstance().getCurrentConnectionStatus();
 *          console.log(connectStatus); => 已连接（实时的连接状态，不一定是已连接）
 */
 ...
 * 获取当前使用的连接通道。
 * 
 * @example var connectChannel =
 *          RongIMClient.getInstance().getConnectionChannel();
 *          console.log(connectChannel); => WebSocket
 *          （正在使用的通道，websocket或者xhrpolling）
 */
 ...
 * 获取当前连接用户的 UserId。
 * 
 * @example var userId = RongIMClient.getInstance().getCurrentUserId();
 *          console.log(userId); => 当前登录人的Id
 */
 ...
 * @param {object}
 *            callback 获取的回调
 * @example RongIMClient.getInstance().clearMessages("conversationType","targetId",{
 *          onSuccess:function(isClear){ // isClear true 清除成功 ， false 清除失败 },
 *          onError:function(){ //清除遇到错误。 } });
 */
...
/**
 * 过滤聊天室消息（拉取最近聊天消息）。 设置位置在 RongIMClient.init() 之后。
 * 
 * @param {array}
 *            msgFilterNames 消息名称数组
 * @example RongIMClient.getInstance().setFilterMessages(["TextMessage"]);
 */
 ...
 * @param {object}
 *            callback 获取的回调
 * @example RongIMClient.getInstance().clearMessagesUnreadStatus("conversationType","targetId",{
 *          onSuccess:function(isClear){ // isClear true 清除成功 ， false 清除失败 },
 *          onError:function(){ //清除遇到错误。 } });
 */
 ...
 * @param {object}
 *            callback获取的回调
 * @example RongIMClient.getInstance().deleteMessages("conversationType","targetId",["messageId1","messageId2"],{
 ...
 * @param {object}
 *            sendCallback
 * @example RongIMClient.getInstance().sendTypingStatusMessage(conversationType,"targetId",RongIMClient.MessageType.TextMessage,{
 *          onSuccess:function(){ console.log("typing Success"); },
 *          onError:function(){ console.log("typing Error"); } });
 */
 ...
 *            <Message>} resultCallback 返回值，函数回调
 * @example RongIMLib.RongIMClient.getInstance().sendMessage("conversationType",
 *          "targetId", "msg对象",{ onSuccess: function(message) {
 ...
 * @param {ResultCallback
 *            <Message>} resultCallback 返回值，函数回调
 * @example RongIMLib.RongIMClient.getInstance().sendTextMessage("conversationType",
 *          "targetId", "消息内容", { onSuccess: function(data) {
 *          console.log(JSON.stringify(data)); //=> data
 *          {messageUId:"消息唯一Id",timestamp:"发送消息时间戳"}
 *          console.log("SendTextMessage Successfully"); }, onError:
 *          function(errorcode) { console.log("SendTextMessage,errorcode:" +
 *          errorcode); } });
 */
 ...
 *            message 消息对象
 * @param {object}
 *            resultCallback 返回值，函数回调
 * @example RongIMLib.RongIMClient.getInstance().sendLocalMessage(message, {
 *          onSuccess: function(message) { console.log("SendTextMessage
 *          Successfully"); }, onError: function(errorcode) {
 ...
 *            content 消息内容
 * @param {ResultCallback
 *            <Message>} callback 返回值，函数回调
 * @example RongIMClient.getInstance().insertMessage("conversationType","targetId","senderUserId","msg对象",{
 *          onSuccess: function() { console.log("insertMessage Successfully"); },
 *          onError: function(errorcode) {
 ...
 *            targetId 用户Id
 * @param {number|null}
 *            pullMessageTime 拉取历史消息起始位置(格式为毫秒数)，可以为null
 * @param {number}
 *            count 历史消息数量
 * @param {ResultCallback
 *            <Message[]>} callback 回调函数
 * @example RongIMLib.RongIMClient.getInstance().getHistoryMessages("conversationType",
 *          "targetId","pullMessageTime", "count", { onSuccess: function(list,
 *          hasMsg) { //list
 *          历史消息数组，hasMsg为boolean值，如果为true则表示还有剩余历史消息可拉取，为false的话表示没有剩余历史消息可供拉取。 },
 *          onError: function(error) { //GetHistoryMessages error } });
 */
...
/**
 * 拉取服务器历史消息记录。 拉取历史消息为循环拉取，举例： 条件：历史记录为45条，每次拉取20条 第一次拉取list长度为20，hasMsg为true。
 * 第二次拉取list长度为20，hasMsg为true。 第三次拉取list长度为5，hasMsg为false。
 * 第四次拉取list长度为0，hasMsg为false。 第四次拉取：重复第一次拉取，以此循环
 * 
 * @param {ConversationType}
 *            conversationType 会话类型
 * @param {string}
 ...
 * @param {ResultCallback
 *            <Message[]>} callback 回调函数
 * @example RongIMLib.RongIMClient.getInstance().getRemoteHistoryMessages("conversationType",
 *          "targetId","pullMessageTime", "count", { onSuccess: function(list,
 *          hasMsg) { //list
 *          历史消息数组，hasMsg为boolean值，如果为true则表示还有剩余历史消息可拉取，为false的话表示没有剩余历史消息可供拉取。 },
 *          onError: function(error) { //getRemoteHistoryMessages error } });
 */
 ...
 *            callback 返回值，参数回调
 * @example RongIMClient.getInstance().hasRemoteUnreadMessages(TOKEN", {
 *          onSuccess: function(hasMsg) { //hasMsg为true表示有未读消息，为false没有未读消息 },
 ...
 * @param {ConnectCallback}
 *            callback 返回值，参数回调
 * @example RongIMClient.getInstance().getTotalUnreadCount({ onSuccess:
 *          function(count) { console.log("count:"+count); }, onError:
 *          function(error) { } });
 */
 ...
 * 
 * @param {ResultCallback
 *            <number>} callback 返回值，参数回调。
 * @param {ConversationType[]}
 *            ...conversationTypes 会话类型。
 * @example RongIMClient.getInstance().getConversationUnreadCount([RongIMLib.ConversationType.PRIVATE],{
 *          onSuccess: function(count) { console.log("count:"+count); },
 *          onError: function(error) { } });
 */
 ...
 * @param {string}
 *            targetId 用户Id
 * @example RongIMClient.getInstance().getUnreadCount("conversationType","targetId",{
 *          onSuccess: function(count) { console.log("count:"+count); },
 *          onError: function(error) { } });
 */
 ...
 *            targetId 目标Id
 * @param {ResultCallback
 *            <boolean>} callback 返回值，参数回调
 * @example RongIMClient.getInstance().clearTextMessageDraft("conversationType","targetId");
 */
 ...
 *            targetId 目标Id
 * @param {ResultCallback
 *            <string>} callback 返回值，参数回调
 * @example var darf =
 *          RongIMClient.getInstance().getTextMessageDraft("conversationType",
 *          "targetId");
 */
 ...
 * @param {ResultCallback
 *            <boolean>} callback 返回值，参数回调
 * @example RongIMClient.getInstance().saveTextMessageDraft("conversationType",
 *          "targetId", "草稿内容");
 */
RongIMClient.prototype.saveTextMessageDraft = function(conversationType,
  ...
  /**
	 ...
	 *            targetId 目标Id
	 * @param {ResultCallback
	 *            <boolean>} callback 返回值，函数回调
	 * @example RongIMClient.getInstance().clearUnreadCount("conversationType","targetId",{
	 *          onSuccess:function(isClear){ console.log(isClear); },
	 *          onError:function(){ } });
	 */
 ...
 *            targetId 目标Id
 * @param {ResultCallback
 *            <Conversation>} callback 返回值，函数回调
 * @example RongIMClient.getInstance().getConversation("conversationType",
 *          "targetId", { onSuccess: function(conver) { //成功 conver
 *          为Conversation对象 }, onError: function(error) { //失败 } });
 */
 ...
 *            callback 返回值，参数回调
 * @param {array}
 *            conversationTypes 可选参数，可以获取指定会话类型的会话，默认请传null
 * @example RongIMClient.getInstance().getConversationList({ onSuccess:
 *          function(list) { //list 会话列表 }, onError: function(error) {
 *          //GetConversationList error } },null);
 * 
 */
 ...
 * @param {ResultCallback}
 *            callback 返回值，参数回调
 * @example RongIMClient.getInstance().getRemoteConversationList({ onSuccess:
 *          function(list) { //list 会话列表 }, onError: function(error) {
 *          //getRemoteConversationList error } },null);
 */
 ...
 * @param {ResultCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().removeConversation("conversationType",
 *          "targetId",{ onSuccess: function(bool) { //bool 是否删除 }, onError:
 *          function(error) { //removeConversation error } });
 */
 ...
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().addMemberToDiscussion("讨论组Id",
 *          ["targetId1","targetId2"], { onSuccess: function() { //加入成功 },
 *          onError: function(error) { //加入失败 } });
 */
 ...
 * @param {string[]}
 *            userIdList 讨论组成员
 * @param {CreateDiscussionCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().createDiscussion("讨论组名称", ["targetId1",
 *          "targetId2"], { onSuccess: function(discussId) { // 创建成功
 *          discussId为讨论组id }, onError: function(error) { // 创建失败 } });
 * 
 */
 ...
 *            discussionId 讨论组Id
 * @param {ResultCallback
 *            <Discussion>} callback 返回值，函数回调
 * @example RongIMClient.getInstance().getDiscussion("讨论组Id", { onSuccess:
 *          function(discuss) { console.log("GetDiscussion successfully"); },
 *          onError: function(error) { console.log("GetDiscussion:errorcode:" +
 *          error); } });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().quitDiscussion("讨论组Id", { onSuccess:
 *          function() { console.log("QuitDiscussion Successfully"); }, onError:
 *          function(error) { console.log("QuitDiscussion:errorcode:" + error); }
 *          });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，参数回调
 * @example RongIMClient.getInstance().removeMemberFromDiscussion("讨论组Id",
 *          "targetId", { onSuccess: function() { console.log("RemoveMember
 *          Successfully"); }, onError: function(error) {
 ...
 * @param {string}
 *            discussionId 讨论组Id
 * @param {DiscussionInviteStatus}
 *            status 邀请状态
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().setDiscussionInviteStatus("讨论组Id",
 *          "讨论组装填", { onSuccess: function() {
 *          console.log("setDiscussionInviteStatus Successfully"); }, onError:
 *          function(error) { console.log("setDiscussionInviteStatus:errorcode:" +
 *          error); } });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().setDiscussionName("讨论组Id", "讨论组名称", {
 *          onSuccess: function() { console.log("setDiscussionName
 *          Successfully"); }, onError: function(error) {
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().joinGroup("群组Id", "群组名称", { onSuccess:
 *          function() { console.log("joinGroup Successfully"); }, onError:
 *          function(error) { console.log("joinGroup:errorcode:" + error); } });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().quitGroup("群组Id", { onSuccess: function() {
 *          console.log("quitGroup Successfully"); }, onError: function(error) {
 ...
 *            chatroomId 聊天室Id
 * @param {number}
 *            messageCount 拉取消息数量，-1为不拉去消息
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().joinChatRoom("聊天室Id", "拉取消息数", {
 *          onSuccess: function() { console.log("joinChatRoom Successfully"); },
 *          onError: function(error) { console.log("joinChatRoom:errorcode:" +
 *          error); } });
 */
 ...
 *            chatroomId 聊天室Id
 * @param {number}
 *            count 获取聊天室用户的人数，范围为0~20
 * @param {GetChatRoomType}
 *            order 按时间排序，范围：0，1，2（0:不返回；1:正序(最早加入)；2:倒序(最晚加入)）
 * @param {ResultCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().getChatRoomInfo("聊天室Id", "拉取消息数" ,"排序方式", {
 *          onSuccess: function(chatRoom) { console.log(chatRoom); //
 *          chatRoom.userInfos => 返回聊天成员。 // chatRoom.userTotalNums => 当前聊天室总人数。 },
 *          onError: function(error) { console.log("getChatRoomInfo:errorcode:" +
 *          error); } });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().quitChatRoom("聊天室Id", { onSuccess:
 *          function() { console.log("quitChatRoom Successfully"); }, onError:
 *          function(error) { console.log("quitChatRoom:errorcode:" + error); }
 *          });
 */
 ...
 * @param {object}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().getRemotePublicServiceList({ onSuccess:
 *          function(list) { console.log(list); }, onError: function(error) {
  ...
  RongIMClient.bridge.queryMsg(28, RongIMLib.MessageUtil.ArrayForm(modules.toArrayBuffer()),
    RongIMLib.Bridge._client.userId, {
      onSuccess: function(data) {
		...
		// MessageUtil.int64ToTimestamp(data.syncTime));
 ...
 * 
 * @param {ResultCallback
 *            <PublicServiceProfile[]>} callback 返回值，参数回调
 * @example RongIMClient.getInstance().getPublicServiceList({
 *          onSuccess:function(list){ console.log(list); }, onError:function(){
 ...
 *            publicServiceType 公众服务类型
 * @param {string}
 *            publicServiceId 公共服务 Id
 * @param {ResultCallback
 *            <PublicServiceProfile>} callback 公共账号信息回调
 * @example RongIMClient.getInstance().getPublicServiceProfile("conversationType","公众帐号Id",{
 *          onSuccess:function(profile){ console.log(profile); },
 *          onError:function(){ cosnole.log("getPublicServiceProfile error"); }
 *          });
 */
 ...
 *            keywords 搜索关键字
 * @param {ResultCallback
 *            <PublicServiceProfile[]>} callback 搜索结果回调
 * @example RongIMClient.getInstance().searchPublicServiceByType("conversationType","searchType","keywords",{
 *          onSuccess:function(list){ console.log(list); }, onError:function(){
 ...
 * @param {OperationCallback}
 *            callback 订阅公众号回调
 * @example RongIMClient.getInstance().subscribePublicService("conversationType","公众账号Id",{
 *          onSuccess:function(){ //关注成功 }, onError:function(){ //关注失败 }
 * 
 * });
 */
 ...
 * @param {OperationCallback}
 *            callback 取消订阅公众号回调
 * @example RongIMClient.getInstance().unsubscribePublicService("conversationType","公众账号Id",{
 *          onSuccess:function(){ //取消关注成功 }, onError:function(){ //取消关注失败 } });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().addToBlacklist("targetId", { onSuccess:
 *          function() { console.log("addToBlacklist Successfully"); }, onError:
 *          function(error) { console.log("addToBlacklist:errorcode:" + error); }
 *          });
 */
 ...
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().getBlacklist({ onSuccess: function(data) {
 *          console.log(data); }, onError: function(error) {
 ...
 *            userId 用户Id
 * @param {ResultCallback
 *            <BlacklistStatus>} callback 返回值，函数回调
 * @example RongIMClient.getInstance().getBlacklistStatus({ onSuccess:
 *          function(data) { console.log(data); }, onError: function(error) {
 *          console.log("getBlacklist:errorcode:" + error); } });
 */
 ...
 * @param {OperationCallback}
 *            callback 返回值，函数回调
 * @example RongIMClient.getInstance().removeFromBlacklist("targetId", {
 *          onSuccess: function() { console.log("removeFromBlacklist
 *          Successfully"); }, onError: function(error) {
 ...
 * 
 * @constructor
 * @example var lm = new RongIMLib.LocationMessage({"content":"地理位置图片的base64内容",
 *          "latitude":"24.114", "longitude":"334.221", "poi":"北京市朝阳区北苑路北辰泰岳大厦",
 *          extra":""});
 */
 ...
 * @constructor
 * @param {object}
 *            RichContentMessage 消息json对象，格式{"title":"News", "content":"I'm
 *            Ironman.", "imageUri":"http://p1.cdn.com/fds78ruhi.jpg", extra:""}
 */
 ...
 * 
 * @example var cnm = new
 *          RongIMLib.ContactNotificationMessage({"operation":"Request",
 *          "sourceUserId":"123", "targetUserId":"456",
 *          "message":"我是小艾，能加一下好友吗？", "extra":""})
 * @constructor
 */
function ContactNotificationMessage(message) {
  ...
  this.user = message.user;
  /**
	 * 同意好友响应。可传入RongIMLib.ContactNotificationMessage对象的setOperation中
	 * 
	 * @static
	 * @type {string}
	 */
  ...
  /**
	 * 拒绝好友响应。可传入RongIMLib.ContactNotificationMessage对象的setOperation中
	 * 
	 * @static
	 * @type {string}
	 */
  ...
  /**
	 * 加好友请求。可传入RongIMLib.ContactNotificationMessage对象的setOperation中
	 * 
	 * @static
	 * @type {string}
	 */
...
/**
 * 资料通知消息. 其中 operation 为资料通知操作，可以自行定义，data 为操作的数据，extra 可以放置任意的数据内容，也可以去掉此属性。
 * 
 * @constructor
 */
 ...
 * 
 * @example var cnm = RongIMLib.CommandNotificationMessage.obtain('命令','去吃饭');
 * @param {string}
 *            name 命令名称
 * @param {string}
 *            data 内容
...
function DiscussionNotificationMessage(message) {
  ...
  /**
	 * 通知类型。 1:加入讨论组 2：退出讨论组 3:讨论组改名 4：讨论组群主T人
	 */
 ...
 * @param {string}
 *            draft 草稿
 * @param {MesssageContent}
 *            latestMessage 最后一条消息
 * @param {string}
 *            latestMessageId 最后一条消息Id
 * @param {ConversationNotificationStatus}
 *            notificationStatus 消息通知状态
 * @param {string}
 *            objectName 内置消息名称
 * @param {number}
 *            receivedTime 消息接收时间
 ...
 *            senderUserId 发送者Id
 * @param {string}
 *            senderUserName 发送者名称
 * @param {SentStatus}
 *            sentStatus 消息发送状态
 * @param {string}
 *            senderPortraitUri 发送者头像
 * @param {number}
 *            sentTime 发送时间
 * @param {string}
 *            targetId 目标Id
 ...
 *            messageUId 消息唯一Id
 * @param {boolean}
 *            hasReceivedByOtherClient 是否是离线消息
 */

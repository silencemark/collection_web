...
var reqPath = "http://slience.yiller.top";
...
function jAjax(url,resData,sucCbk,errCbk,notLoad){
	if(!notLoad && (!$("#loadingWindow") || $("#loadingWindow").length==0)){
  ...
  console.log("request to "+reqPath + url);
  console.log(resData);
	$.ajax({
		type : "POST",
		dataType:'json',
...
function chkUrlISWhite(url){
	var whiteList = [
		...
		"/appIndex/getNewSystemNotice",
		"/appVipCard/getGarageKitList",
		"/appVipCard/getGarageKitInfo",
	];
...
$(function(){
  ...
  if((!$("#ntMsg") || !$("#ntMsg").val()) && getCptToken()){
    lxQuserWMsg();
  }
});

//轮询
function lxQuserWMsg(){
  if(isQueryMsg){
      jAjax("/appUserCenter/getUserNotice",{status:0},function(data){
          if(data.resultlist && data.resultlist.length>0){
              isQueryMsg = false;
              clearInterval(msgInter);
              setMsgHtml(data.resultlist.length,data.resultlist[0]);
          }
      },null,true);
      if(!msgInter && isQueryMsg){
...
var showNoticeid ;
function setMsgHtml(sumLen,msg){
  showNoticeid = msg.noticeid;
  var msgTitle = msg.title;
  var msgCont = msg.message;
  var msgHtml = 
  '<div class="msgLoad apdMsg"></div>'+
  '<div class="msgLoadCont  apdMsg">'+
    '<img src="'+LOCURL+'img/defaulttx.png"/>'+
    '<span class="msgTitleSpan">'+sumLen+'条未读消息</span>'+
    '<div class="msgWinCont" onclick="msgReadAll(1);" >'+
        '【'+msgTitle+'】 '+msgCont+
    '</div>'+
    '<div class="mgsBUTred">'+
          '<span onclick="msgReadAll(0);">标记已读</span>'+
    '</div>'+
  '</div>';
  $("body").append(msgHtml);
}

function msgReadAll(tp,msg){
  jAjax("/appUserCenter/updateNoticeStatus",{status:1},function(data){});
  $(".apdMsg").hide(300);
  isQueryMsg = false;
  clearInterval(msgInter);
  if(tp==1){
      window.location.href=LOCURL+'user/userMsg.html?showNoticeid='+showNoticeid;
  }
}

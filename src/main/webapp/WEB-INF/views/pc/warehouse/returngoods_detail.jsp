...
var relayid = '${returnInfo.orderid}';
var relaytype = 4;

function exportexcel(){
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportReturnInfo",
        data: 'orderid=${returnInfo.orderid}',
...
$(document).ready(function(){
	...
	$('#return').attr('class','active');
	$('#homepage').parent().find("li").attr('class','');
	$('#homepage').attr('class','active');
})
function onloaddata(){
	readstatus('${returnInfo.orderid}','${userInfo.userid}');
	onloadDate();
}
...
<div class="page_main">
    ...
    <div class="right_page">
   	  <div class="page_name"><span>退货单详情</span><a href="javascript:void(0)" onclick="goBackPage()" class="back">返回</a>
   	  <a href="javascript:void(0)" onclick="exportexcel();">导出</a>
   	  <c:if test="${returnInfo.examineuserid!=userInfo.userid || returnInfo.status!=0}">
...
function pageHelper(num){
	...
	param.orderid='${returnInfo.orderid}';
	param.resourcetype= 4;
	pageList(param);
}
...
function checkComment(row){
	...
	param.resourceid = '${returnInfo.orderid}';
	param.resourcetype =4;
	showComment(row,param,'/pc/getReturnInfo?orderid=${returnInfo.orderid}&resourcetype=4');//添加评论信息
}
...
function examineorder(result){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:'<%=request.getContextPath()%>/app/examineReturnOrder',
		data:'orderid=${returnInfo.orderid}&userid=${userInfo.userid}&result='+result+'&opinion='+$('#opinion').val(),
		success:function(data){
				swal({
					title : "",
					text : "操作成功",
					...
					changeIsread('${returnInfo.createid}','${returnInfo.orderid}');
					var title="${returnInfo.examinename }审批了您的退货单,请及时查看";
					var url="/warehouse/return_detail.html?orderid=${returnInfo.orderid}";
					pushMessage('${returnInfo.createid}',title,url);
					goBackPage();
				});
			}
	})

...
function choosetype(){
	...
	typeparam.type = type;
	typeparam.companyid = "${userInfo.companyid}";
	typeparam.organizeid = $('#organizeid').val();
	typeparam.groupname = 1;
	$.ajax({
		type:'post',
		dataType:'json',
		url:'<%=request.getContextPath()%>/app/getPurchaseMaterial?',
		data:typeparam,
		success:function(data){
			if(data.status==0){
...
function choosename(){
	...
	typename.type = type;
	typename.companyid = "${userInfo.companyid}";
	typename.organizeid = $('#organizeid').val();
	typename.name = name;
	typename.groupspecifications = 1;
...
function choosespecifications(){
	...
	typeparam.organizeid = $('#organizeid').val();
	typeparam.name = name;
	typeparam.specificationsall = specifications;
...
function changenum(obj){
	...
	$('#addul select[name=price] option').each(function(index,item){
		stocknum+=parseFloat($(item).attr("stock"));
		
		stock1=parseFloat($(item).attr("stock"));
		if(index ==0){
			materielid+=$(item).attr("materielid");
		}else{
			if(num1 > stock1 ){
				materielid+="_"+$(item).attr("materielid");
				num1=num1-stock1;
			}else if(num1 > 0){
				materielid+="_"+$(item).attr("materielid");
				num1=0;
	...
	if($('#materialul input[value='+materielid+']').parent().parent().find('em[name=num]').length>0){
		$('#materialul input[value='+materielid+']').parent().parent().find('em[name=num]').each(function(index,item){
			num+=parseFloat($(item).text());
		})
	}
	...
	$('#addul select[name=price] option').each(function(index,item){
		stock=parseFloat($(item).attr("stock"));
		if(num > stock){
			num=num-stock;
		}else{
...
<div class="page_main">
    ...
    <div class="right_page">
    	<div class="page_name"><span>新建用料单</span><a href="<%=request.getContextPath() %>/pc/getMaterialOrder" class="back">返回</a></div>
            ...
            <div class="tab_list">
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="none_border">
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="usestarttime" class="text" style="width:100px;" readonly="readonly" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>--<input type="text" readonly="readonly" style="width:100px;" id="useendtime" class="text"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"/></td>
                    </tr>
                    <tr>
...
<script type="text/javascript">
$(function(){
	$("#usestarttime").val(GetDateStr(0));
	$("#useendtime").val(GetDateStr(0));
});
...
function submitdata(){
	...
	var orderno='${orderno}';
	var organizeid=$('select[name=organizeid]').val();
	var url='<%=request.getContextPath()%>/app/insertMaterial';
	...
	param.usetime = $("usestarttime").val()+" - "+$("#useendtime").val();
		...
		success:function(data){
			if(data.status==1){
				...
				swal({
					...
					location.href="<%=request.getContextPath() %>/pc/getMaterialOrder";
				});
			}
		},error:function(text){
			alert("添加失败");

...
var relayid = '${reportlossInfo.orderid}';
var relaytype = 5;

function exportexcel(){
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportReportLossInfo",
        data: 'orderid=${reportlossInfo.orderid}',
...
function onloaddata(){
	readstatus('${reportlossInfo.orderid}','${userInfo.userid}');
	onloadDate();
}
...
<div class="page_main">
    ...
    <div class="right_page">
   	  <div class="page_name"><span>报损单详情</span><a href="javascript:void(0)" onclick="goBackPage()" class="back">返回</a>
   	  <a href="javascript:void(0)" onclick="exportexcel();">导出</a>
   	  <c:if test="${reportlossInfo.examineuserid!=userInfo.userid || reportlossInfo.status!=0}">
   	  <a href="javascript:void(0)"  onclick="showForwardDiv()" >转发</a>
   	  </c:if>
   	  </div>
        <div class="page_tab2">  
          <c:if test="${reportlossInfo.result==1}">
          <div class="check_state"><img src="../userbackstage/images/pc_page/agree_img.png" /></div>        
          </c:if>  
          <c:if test="${reportlossInfo.result==0}">
          ...
          <div class="tab_list">
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        ...
                        <td>${reportlossInfo.orderno}</td>
                    </tr>
                    <tr>
                    	<td class="l_name">店面</td>
                        <td>${reportlossInfo.organizename}</td>
                    </tr>
                    <tr>
                        ...
                        <td class="mx_tab">
                        	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mx_td">
                                ...
                                <tr class="none_border">
                                	<td colspan="4">共 <i class="yellow">${reportlossInfo.maternum }</i> 项</td>
                                    <td colspan="3" class="t_r">合计<i class="yellow">￥${reportlossInfo.materialprice }</i>元</td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td class="img_td"><div class="img f"><img src="${reportlossInfo.createheadimage}" width="30" height="30" /></div><i class="i_name">${reportlossInfo.createname}</i></td>
                    </tr>
                    <tr>
                    	<td class="l_name">审批人</td>
                        <td class="img_td"><div class="img f"><img src="${reportlossInfo.examineheadimage}" width="30" height="30" /></div><i class="i_name">${reportlossInfo.examinename }</i></td>
                    </tr>
                    <tr>
                    	<td class="l_name">报损时间</td>
                        <td>${reportlossInfo.reportlosstime }</td>
                    </tr>
                    <tr>
                    	<td class="l_name">填写时间</td>
                        <td>${reportlossInfo.createtime }</td>
                    </tr>
                    <tr>
                    	<td class="l_name">抄送人</td>
                        <td class="img_td" id="CCusernames">
                        	<c:forEach items="${reportlossInfo.ccuserlist }" var="map">
                    ...
                    <tr>
                        ...
                        <td>${reportlossInfo.updatetime }</td>
                    </tr>
                    <c:choose>
                    	<c:when test="${reportlossInfo.examineuserid==userInfo.userid && reportlossInfo.status==0}">
	                    <tr class="none_border">
...
function pageHelper(num){
	...
	param.orderid='${reportlossInfo.orderid}';
...
function checkComment(row){
	...
	param.resourceid = '${reportlossInfo.orderid}';
	param.resourcetype =5;
	showComment(row,param,'/pc/getReportlossInfo?orderid=${reportlossInfo.orderid}&resourcetype=5');//添加评论信息
}
...
function examineorder(result){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:'<%=request.getContextPath()%>/app/examineReportlossOrder',
		data:'orderid=${reportlossInfo.orderid}&userid=${userInfo.userid}&result='+result+'&opinion='+$('#opinion').val(),
		success:function(data){
				swal({
					title : "",
					text : "操作成功",
					...
					changeIsread('${reportlossInfo.createid}','${reportlossInfo.orderid}');
					var title="${reportlossInfo.examinename }审批了您的报损单,请及时查看";
					var url="/warehouse/reportloss_detail.html?orderid=${reportlossInfo.orderid}";
					pushMessage('${reportlossInfo.createid}',title,url);
					goBackPage();
				});
			}
	})

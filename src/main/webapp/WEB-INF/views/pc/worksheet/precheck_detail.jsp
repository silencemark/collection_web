...
<script type="text/javascript">
	...
	function onloaddata() {
		...
		readstatus('${precheckinfo.inspectid}', '${userInfo.userid}');
	}
function exportexcel(){
	...
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportBeforeMealDetailList",
        data: 'inspectid=${precheckinfo.inspectid}',
...
<jsp:include page="../top.jsp"></jsp:include><body>

<div class="page_main">
	<jsp:include page="left.jsp"></jsp:include>
    <div class="right_page">
    	<div class="page_name"><span>餐前检查单详情</span><a href="javascript:void(0)" onclick="window.history.go(-1)" class="back">返回</a>
    	<a href="javascript:void(0) " onclick="exportexcel()">导出</a>
    	<c:if test="${precheckinfo.examineuserid!=userInfo.userid || precheckinfo.status!=0 }">
            ...
            <div class="tab_list">
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="none_border">
                    <tr>
                        ...
                        <td>${precheckinfo.organizename}</td>
                    </tr>
                    <tr class="last_td">
                    	<td class="l_name">检查区域</td>
                        <td>${precheckinfo.templatename}</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="img_td"><div class="img f"><img src="${precheckinfo.createheadimage}" width="30" height="30" /></div><i class="i_name">${precheckinfo.createname}</i></td>
                    </tr>
                    <tr>
                    	<td class="l_name">填写时间</td>
                        <td>${precheckinfo.createtime}</td>
                    </tr>
                    <tr>
                    	<td class="l_name">审核人</td>
                        <td class="img_td"><div class="img f"><img src="${precheckinfo.examineheadimage}" width="30" height="30" /></div><i class="i_name">${precheckinfo.examinename}</i></td>
                    </tr>
                    <tr>
                    	<td class="l_name">抄送人</td>
                        <td class="img_td">
                        	<c:forEach items="${precheckinfo.ccuserlist }" var="map">
                    ...
                    <tr>
                     ...
                     <c:choose>
                    	<c:when test="${precheckinfo.examineuserid==userInfo.userid && precheckinfo.status==0 }">
	                    <tr class="none_border">
...
<script type="text/javascript">
	...
	function checkComment(row) {
		...
		showComment(row, param,
				'/pc/getBeforeMealInfo?inspectid=${precheckinfo.inspectid}&resourcetype=11');//添加评论信息
	}
 function examineorder(result){
		...
		$.ajax({
			type:'post',
			dataType:'json',
			url:'<%=request.getContextPath()%>/pc/examineBeforeMeal',
			data:'inspectid=${precheckinfo.inspectid}&userid=${userInfo.userid}&result='+result+'&opinion='+$('#opinion').val(),
			success:function(data){
					swal({
						title : "",
						text : "操作成功",
						...
						location.href='<%=request.getContextPath() %>/pc/getBeforeMealList';
					});
				}
		})
	}
...
<script type="text/javascript">
$(function(){
	var starlist = '${precheckinfo.starlist}';

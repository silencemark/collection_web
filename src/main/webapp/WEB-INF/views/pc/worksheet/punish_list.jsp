...
function exportexcel(){
	...
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportPunishList",
...
<body>
	...
	<jsp:include page="left.jsp"></jsp:include>
		<div class="right_page">
			<div class="page_name">
				<span>处罚单列表</span><a href="javascript:void(0)"
					onclick="exportexcel()">导出</a>
					<c:if test="${powerMap.power4001010 != null}"><a href="<%=request.getContextPath() %>/pc/intoPunishAddPage">新建</a></c:if>
			</div>
			<div class="page_tab">
				<div class="sel_box">
					<form action="<%=request.getContextPath()%>/pc/getPunishListInfo"
						id="searchform"> <input type="text" class="text_time"
				...
				<div class="tab_list">
					...
					<c:forEach var="item" items="${Punishlist}">
						<tr>
							<td>${item.realname}</td>
							...
							<td><a href="<%=request.getContextPath() %>/pc/getPunishDetailInfo?punishid=${item.punishid}&forwarduserid=${item.id}" class="link">查看</a>
					...
					<c:if test="${fn:length(Punishlist) == 0 }">
	                	<c:if test="${powerMap.power4001010 != null}">
	                		<div class="list_none"><span><i class="yellow">Hello,我是大狮！</i><br>还没有处罚单，赶紧添加一条吧~</span><b><img src="<%=request.getContextPath() %>/userbackstage/images/index/none_msg.gif" width="293" height="240"></b></div>
	                	</c:if>
	                	<c:if test="${powerMap.power4001010 == null}">
	                		<div class="list_none"><span><i class="yellow">Hi,我是大狮！</i><br>列表空空，还没有处罚单呢~</span><b><img src="<%=request.getContextPath() %>/userbackstage/images/index/none_msg2.gif" width="293" height="240"></b></div>
	                	</c:if>
	                </c:if>
				</div>

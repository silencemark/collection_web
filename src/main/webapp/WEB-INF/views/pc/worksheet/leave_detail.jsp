...
function exportexcel(){
	...
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportLeaveShopDetailList",
        data: 'inspectid=${leaveinfo.inspectid}',
...
function onloaddata() {
	...
	readstatus('${leaveinfo.inspectid}', '${userInfo.userid}');
}
...
<div class="page_main">
    ...
    <div class="right_page">
    	<div class="page_name"><span>离店报告单详情</span><a href="javascript:void(0)" onclick="window.history.go(-1)" class="back">返回</a>
    	<a href="javascript:void(0)" onclick="exportexcel()">导出</a>
    	<c:if test="${leaveinfo.examineuserid!=userInfo.userid || leaveinfo.status!=0 }">
            ...
            <div class="tab_list">
                <table width="100%" border="0" cellpadding="0" cellspacing="0" class="none_border">
                    <tr>
                        ...
                        <td>${leaveinfo.organizename}</td>
                    </tr>
                    <tr class="last_td">
                    	<td class="l_name">检查区域</td>
                        <td>${leaveinfo.templatename}</td>
                    </tr>
                        ...
                        <td class="mx_tab">
                        	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mx_td">
                                ...
                                <tr class="none_border">
                                    ...
                                    <td class="t_r">
                                    	<div class="star_box">
									    	<span><i style="display: none;" id="sumstarnum">0</i></span>
										    <li class="n_bor">
											    <b>
                    ...
                    <tr>
                        ...
                        <td class="img_td"><div class="img f"><img src="${leaveinfo.createheadimage}" width="30" height="30" /></div><i class="i_name">${leaveinfo.createname}</i></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="img_td"><div class="img f"><img src="${leaveinfo.examineheadimage}" width="30" height="30" /></div><i class="i_name">${leaveinfo.examinename}</i></td>
                    </tr>
                    <tr>
                    	<td class="l_name">抄送人</td>
                        <td class="img_td">
                        	<c:forEach items="${leaveinfo.ccuserlist }" var="map">
                     ...
                     <tr>
                        ...
                        <c:choose>
                    	<c:when test="${leaveinfo.examineuserid==userInfo.userid && leaveinfo.status==0 }">
	                    <tr class="none_border">
	                    	<td class="l_name2">审核意见</td>
...
function checkComment(row) {
	...
	showComment(row, param,
			'/pc/getLeavedetailInfo?inspectid=${leaveinfo.inspectid}&resourcetype=10');//添加评论信息
}
 ...
 function examineorder(result){
		...
		$.ajax({
			type:'post',
			dataType:'json',
			url:'<%=request.getContextPath()%>/pc/examineLeaveShop',
			data:'inspectid=${leaveinfo.inspectid}&userid=${userInfo.userid}&result='+result+'&opinion='+$('#opinion').val(),
			success:function(data){
					swal({
						title : "",
						text : "操作成功",
						...
						location.href='<%=request.getContextPath() %>/pc/getLeaveShopListInfo';
					});
				}
		})
	}
...
<script type="text/javascript">
$(function(){
	var starlist = '${leaveinfo.starlist}';

...
<script type="text/javascript">
	...
	function onloaddata() {
		...
		readstatus('${costinfo.dishesid}', '${userInfo.userid}');
	}
</script>
<script type="text/javascript">
function exportexcel(){
	...
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportDishesDetailList",
        data: 'dishesid=${costinfo.dishesid}',
...
<body onload="onloaddata()">
	...
	<div class="page_main">
		...
		<div class="right_page">
			<div class="page_name">
				<span>菜品成本单详情</span><a href="/pc/getcostListInfo" class="back">返回</a><a
					href="javascrpit:void(0)" onclick="exportexcel()">导出</a>
					<c:if test="${costinfo.examineuserid!=userInfo.userid || costinfo.status!=0 }">
				...
				<div class="tab_list">
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td>${costinfo.dishestypename}</td>
						</tr>
						<tr>
							<td class="l_name">菜品名称</td>
							<td>${costinfo.dishesname}</td>
						</tr>
						<tr>
							<td class="l_name">投料标准</td>
							<td>${costinfo.feeding}</td>
						</tr>
						<tr>
							<td class="l_name">成本价</td>
							<td>${costinfo.costprice}</td>
						</tr>
						<tr>
							<td class="l_name">售价</td>
							<td>${costinfo.price}</td>
						</tr>
						<tr>
							<td class="l_name">成本率</td>
							<td>${costinfo.costrate}</td>
						</tr>
						<tr class="last_td">
							<td class="l_name2">地点及描述</td>
							<td>${costinfo.organizename}
								<div class="clear"></div>
						...
						<tr>
							...
							<td class="img_td"><div class="img f">
									<img src="${costinfo.createheadimage}" width="30" height="30" />
								</div>
								<i class="i_name">${costinfo.createname}</i></td>
						</tr>
						<tr>
							<td class="l_name">填写时间</td>
							<td>${costinfo.createtime}</td>
						</tr>
						<tr>
							<td class="l_name">审核人</td>
							<td class="img_td"><div class="img f">
									<img src="${costinfo.examineheadimage}" width="30" height="30" />
								</div>
								<i class="i_name">${costinfo.examinename}</i></td>
						</tr>
						<tr>
                    	<td class="l_name">抄送人</td>
                        <td class="img_td">
                        	<c:forEach items="${costinfo.ccuserlist }" var="map">
						...
						<tr>
	                        ...
	                        <td class="img_td">
	                        	<i class="i_name" id="sendname"></i>
						            ...
						            <c:choose>
                    	<c:when test="${costinfo.examineuserid==userInfo.userid && costinfo.status==0 }">
			...
			<div class="comment">
				<div class="name">
					共有<i class="yellow" id="num"></i>条评论
				</div>
				<div class="text_box">
				<c:if test="${costinfo.examineuserid!=userInfo.userid || costinfo.status!=0 }">
					<b><textarea placeholder="请输入评论内容，最多允许输入800字符" maxlength="800" 
							id="content_text1"></textarea></b> <span><input type="button"
						value="评论" onclick="checkComment(1)" /></span> 
					</c:if>
						<input type="hidden"
						id="parentid" /> <input type="hidden" id="parentuserid" />
				</div>
				<div class="ul_list" id="ul_list"></div>
				<div id="Pagination" style="width: 315px;">${pager}</div>
				<!--动态的获取pagination的宽度赋值给Pagination-->
				<div class="clear"></div>
			</div>
		</div>
		<div class="clear"></div>
...
<script type="text/javascript">
	//显示发布的语音和图片
	$(function() {
		var filelist = '${costinfo.filelist}';
	...
	function checkComment(row) {
		...
		param.resourceid = '${map.dishesid}';
		param.resourcetype = 14;
		showComment(row, param, '/pc/getcostDetailInfo?dishesid=${costinfo.dishesid}&resourcetype=14');//添加评论信息
	}
 ...
 function examineorder(result){
		...
		$.ajax({
			type:'post',
			dataType:'json',
			url:'<%=request.getContextPath()%>/pc/updateDishesInfo',
			data:'dishesid=${costinfo.dishesid}&userid=${userInfo.userid}&result='+result+'&opinion='+$('#opinion').val(),
			success:function(data){
					swal({
						title : "",
						text : "操作成功",
						...
						location.href='<%=request.getContextPath() %>/pc/getcostListInfo';
					});
				}
		})
	}

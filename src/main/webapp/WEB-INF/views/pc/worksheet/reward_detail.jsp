...
<body onload="onloaddata()">
	...
	<div class="page_main">
		...
		<div class="right_page">
			<div class="page_name">
				<span>奖励单详情</span><a href="/pc/getRewardListInfo" class="back">返回</a><a
					href="javascript:void(0)" onclick="exportexcel()">导出</a>
					<c:if test="${rewardinfo.examineuserid!=userInfo.userid || rewardinfo.status!=0}">
...
<!-- 									<div class="clear"></div> -->
						...
						<tr>
	                        ...
	                        <td class="img_td" id="CCusernames">
	                        	<c:forEach items="${rewardinfo.ccuserlist }" var="map">
						...
						<c:choose>
							<c:when test="${rewardinfo.examineuserid==userInfo.userid && rewardinfo.status==0}">
		...
		if (filelist.length > 0 && filelist != null) {
			...
			$.each(filelist,function(i, map) {
				if (map.type == 1) {
					...
					imghtml += '<div class="talk" onclick="PlayRecordPC(this,\''+map.visiturl+'\')">语言文件</div>';
				}
			});
			//imghtml += '<div class="talk" onclick="PlayRecordPC(this,\'/upload/qrcodes/123456.mp3\')">语言文件</div>';

...
var relayid = '${purchaseInfo.orderid}';
var relaytype = 1;

function exportexcel(){
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath() %>/pc/exportPurchaseInfo",
        data: 'orderid=${purchaseInfo.orderid}',
...
function onloaddata(){
	readstatus('${purchaseInfo.orderid}','${userInfo.userid}');
	onloadDate();
}
...
<div class="page_main">
    ...
    <div class="right_page">
   	  <div class="page_name"><span>采购(入库)单详情</span><a href="javascript:void(0)" onclick="goBackPage()" class="back">返回</a>
   	  <a href="javascript:void(0)" onclick="exportexcel();">导出</a>
   	  <c:if test="${purchaseInfo.examineuserid!=userInfo.userid || purchaseInfo.status!=0 }">
...
function pageHelper(num){
	...
	param.orderid='${purchaseInfo.orderid}';
...
function checkComment(row){
	...
	param.resourceid = '${purchaseInfo.orderid}';
	param.resourcetype =1;
	showComment(row,param,'/pc/getPurchaseInfo?orderid=${purchaseInfo.orderid}&resourcetype=1');//添加评论信息
}
...
function examineorder(result){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:'<%=request.getContextPath()%>/app/examinePurchaseOrder',
		data:'orderid=${purchaseInfo.orderid}&userid=${userInfo.userid}&result='+result+'&opinion='+$('#opinion').val(),
		success:function(data){
				swal({
					title : "",
					text : "操作成功",
					...
					changeIsread('${purchaseInfo.createid}','${purchaseInfo.orderid}');
					var title="${purchaseInfo.examinename}审批了您的采购(入库)单,请及时查看";
					var url="/purchase/purchase_detail.html?orderid=${purchaseInfo.orderid}";
					pushMessage('${purchaseInfo.createid}',title,url);
					goBackPage();
				});
			}
	})

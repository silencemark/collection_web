 				...
 				if(data.datalist!=null){
 					...
 					for(var i= 0;i<data.datalist.length;i++){
 						temp+="<tr>"+
	 	                    	...
	 	                    	if(data.datalist[i].urgentlevel==1){
	 	                    		 temp+=" <td><i class=\"green\">普通</i></td>";
	 	                    	}else if(data.datalist[i].urgentlevel==2){
	 	                    		 temp+=" <td><i class=\"yellow\">紧急</i></td>";
	 	                    	}else if(data.datalist[i].urgentlevel==3){
	 	                    		 temp+=" <td><i class=\"red\">非常紧急</i></td>";
	 	                    	}else{
	 	                    		 temp+=" <td></td>";
	 	                    	}
	 	                     
	 	                    	temp+=  "<td>"+data.datalist[i].reason+"</td>"+
	 	                       ...
	 	                       temp += "<td><a href=\"#\" class=\"link\" onclick=\"checklook('"+data.datalist[i].reserveamountid+"','"+data.datalist[i].isread+"','"+data.datalist[i].forwarduserid+"')\">查看</a>";
	 	                       if(data.datalist[i].status == 0){
	 	                    		if(data.datalist[i].examineuserid == '${userInfo.userid}'){
	 	                      		 temp +="<a href=\"#\" class=\"yellow\" onclick=\"checkexamine('"+data.datalist[i].reserveamountid+"','"+data.datalist[i].isread+"')\">审批</a>";
	 	                      		}	
	 	                       }else{
...
var nodata = '<div class="list_none"><span><i class="yellow">Hello,我是大狮！</i><br>还没有备用金申请单，赶紧添加一个吧~</span><b><img src="/userbackstage/images/index/none_msg.gif" width="293" height="240"></b></div>';
function onloadDate(){
	$.ajax({
 		type:'post',
 		url:'/pc/getReserveAmountList',
...
function checklook(reserveamountid,isread,forwarduserid){
	intoDetail("/pc/getPcOaReserveDetail?reserveamountid="+reserveamountid+"&forwarduserid="+forwarduserid,reserveamountid,isread,20);
}

function checkexamine(reserveamountid,isread){
	intoDetail("/pc/getPcOaReserveCheck?reserveamountid="+reserveamountid,reserveamountid,isread,20);
}
...
function exportexcel(){
	 swal({
			...
			$.ajax({
		        type: "POST",
		        url: "<%=request.getContextPath() %>/pc/exportReserveList",
		        data:param,
		        success: function(data){
...
<jsp:include page="left.jsp"></jsp:include>
    <div class="right_page">
    	<div class="page_name"><span>备用金申请列表</span><a href="#" onclick="exportexcel()">导出</a>
    	<a href="<%=request.getContextPath() %>/pc/getPcOaReserveAdd">新建</a>
    	</div>
        <div class="page_tab">
            <div class="sel_box">

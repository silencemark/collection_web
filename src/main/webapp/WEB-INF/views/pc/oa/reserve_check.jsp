	...
	success:function(data){
		if(data.status==1){
			...
			if(data.reserveamountinfo!=null){
				...
				}else if(data.reserveamountinfo.result==2){
				...
				examine_createid = data.reserveamountinfo.createid;
...
function examineInfo(result){
	var param = new Object();
	param.reserveamountid ='${map.reserveamountid}';
	param.userid = '${userInfo.userid}';
	param.opinion = $('#opinion').val();
	param.result = result;
	
	$.ajax({
		url:"/pc/examineReserveAmount",
		type:"post",
		data:param,
		success:function(resultMap){
			if(resultMap.status == 0 || resultMap.status == '0'){
				changeIsread(examine_createid,"${map.reserveamountid}");
				swal({
					title : "",
					text : "审批成功！",
					type : "success",
...
<jsp:include page="left.jsp"></jsp:include>
    <div class="right_page">
    	<div class="page_name"><span>备用金详情</span>
    	<a href="<%=request.getContextPath() %>/pc/getPcOaReserveList" class="back">返回</a><a href="#" onclick="exportexcel()">导出</a>

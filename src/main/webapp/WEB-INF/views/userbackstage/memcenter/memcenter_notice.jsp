...
function selectTitle(){
	var userid="${userid}";
	var title = $("#title").val();
	location.href="/userbackstage/getSystemMessageListByUser?title="+title+"&userid="+userid;
	
}

	
$(document).ready(function(){
	$('#nav_notice').attr('class','active');$('#nav_notice').parent().parent().show();
})
</script>
</head>
...
<div class="main_page">
    ...
    <div class="page_tab m_top">
        <div class="tab_name"><span class="gray1">系统公告</span></div>
        <div class="sel_box">
        ...
        <div class="tab_list">
        	<table width="100%" border="0" cellpadding="0" cellspacing="0"  id="messageTable" style="font-size: 12px;">
                ...
                <c:forEach var="list" items="${dataList}">
	               	<tr>
	                	<td>${list.title}</td>
	                    <td>${fn:substring(list.content,0,(fn:indexOf(list.content,"。")+1))}</td>
	                    <td>${list.createtime}</td>
	                    <td><a href="<%=request.getContextPath()%>/userbackstage/getSystemMessageNoticeById?messageid=${list.messageid}" class="blue">查看</a></td>
	                </tr>      
                </c:forEach>

...
function onloadData(){
	if('${errors}' != ''){
	 ...
	 if(sex!=null && sex!=undefined && sex!="" && sex == 1 ){
		 $("#sex").html("男");
	 }else if(sex!=null && sex!=undefined && sex!="" && sex == 0){
		 $("#sex").html("女");
	 }
}
...
function checkUpdata(isNum){
	...
	if(isNum == 1){
	 ...
	 $.ajax({
	 		type:'post',
	 		dataType:'json',
	 		url:'/userbackstage/updateOrganizeUserInfo?isshowphone='+isshowphone+"&userid=${userinfo.userid}",
...
function checkUpdataP(){
	...
	if(num == 1){
		location.href='/userbackstage/updataPhoen?u_phone='+u_phone+'&V_Code='+V_Code+'&phone='+phone+'&C_Code='+C_Code;
	}else{
		swal({
			title : "",
			text : mess,
...
function checkUpdataE(){
	...
	if(num == 1){
		location.href='/userbackstage/updataEmail?email='+email;
	}else{
		swal({
			title : "",
			text : mess,
...
<div class="main_page">
    ...
    <div class="page_tab">
        <div class="tab_name"><span class="gray1">个人信息</span><a href="<%=request.getContextPath() %>/userbackstage/getPassword" class="wid_01">重置密码</a><a href="<%=request.getContextPath()%>/userbackstage/selectUpdataUserInfo">修改</a></div>
        <div class="tab_list">
        	<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size: 12px;">
                ...
                <tr>
                  ...
                  <i class="info_wid" id="phone">
                  </i>
                  <a href="#" class="info_btn"  onclick="checkOpen(1)">更换手机号</a><div class="info_xx">
                  <a href="#" class="radio_ed" id="kai" onclick="checkUpdata('1')">选择</a>
                  <i class="i_dp" >公开</i>
                  <a href="#" class="radio" id="bu" onclick="checkUpdata('0')">选择</a>
                  <i class="i_dp">隐藏</i></div></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="2" >${userinfo.position}</td>
                </tr>
            </table>
        </div>
    </div>

...
function getdata(){
	...
	$.ajax({
		...
		url:projectpath+'/app/getReportConsumptionChart?organizeid='+organizeid,
			...
			if(data.status==0){
				...
				for(var i=0;i<data.nowmonthlist.length;i++){
					...
					if(firstcount<i && i<nextcount){
						var rjxf = data.nowmonthlist[i].sumdata+"";
						rjxf = rjxf.substring(0,(rjxf.indexOf(".")+3));
						nowdataArray[i]=rjxf;
						nowsumrjxf += parseInt(rjxf);
					}
					/* timedataArray[i]=data.nowmonthlist[i].comparetime;
					var rjxf = data.nowmonthlist[i].sumdata+"";
					rjxf = rjxf.substring(0,(rjxf.indexOf(".")+3));
				...
				for(var i=0;i<data.lastmonthlist.length;i++){
					if(firstcount2<i && i<nextcount2){
						var rjxf = data.lastmonthlist[i].sumdata;
						rjxf = rjxf.substring(0,(rjxf.indexOf(".")+3));
						lastdataArray[i] = rjxf;
						lastsumrjxf += parseInt(rjxf);
					}
...
// 				}
				...
				if(nowreportnum > 0){
					nowsumrjxf = nowsumrjxf/parseInt(nowreportnum) + "";
					nowsumrjxf = nowsumrjxf.substring(0,(nowsumrjxf.indexOf(".")+3));
				}
				if(lastreportnum > 0){
					lastsumrjxf = lastsumrjxf/lastreportnum + "";
					lastsumrjxf = lastsumrjxf.substring(0,(lastsumrjxf.indexOf(".")+3));
				}
				...
				option = {
					    ...
					    legend: {
					    	orient:'vertical',
					        data:[data.lastmonth+"月，人均消费："+lastsumrjxf+"元",data.nowmonth+"月，人均消费："+nowsumrjxf+"元"]
					    },
					    grid: {
					    	x:50,y:60,
					        y2:40,x2:20
...
// 					            rotate:40,
					    ...
					    series: [
					        {
					            name:data.lastmonth+"月，人均消费："+lastsumrjxf+"元",
					            type:'line',
					            data:lastdataArray,
					            symbol:'circle'
					        },
					        {
					            name:data.nowmonth+"月，人均消费："+nowsumrjxf+"元",
					            type:'line',
					            data:nowdataArray,
					            symbol:'circle'
					        }
...
<div class="main_page">
    ...
    <div class="right_page">
    	<div class="page_nav"><p><a href="#">首页</a><i>/</i><a href="<%=request.getContextPath() %>/managebackstage/getCompanyList">企业信息列表</a><i>/</i><a href="<%=request.getContextPath() %>/managebackstage/getCompanyInfo?companyid=${map.companyid}">公司信息</a><i>/</i><span>报表管理/人均消费</span></p></div>
        <div class="page_tab2">        	
            ...
            <div class="mrbb_nav">
                 <ul>
                    ...
                    <li class="active"><a href="<%=request.getContextPath()%>/managebackstage/getReportConsumptionChart?companyid=${map.companyid}">人均消费</a></li>
            ...
            <div class="mrbb_chartbox">
            	<div class="time_sel">
                    <b style="background:#eee;"><i class="pre" onclick="preMonth()">前一天</i><input style="border:0px; background-color:#eee;" id="statisticsTime" readonly="readonly" onchange="getdata()" value="请选择时间"><i onclick="nextMonth()" class="next">后一天</i></b>
                </div>

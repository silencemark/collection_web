...
function addproject(id,name){
	var project = '<li class="n_bor" id="'+id+'_project_li">'+
				    	...
				    	'<span>'+name+'</span>'+
				    	'<b>'+
							'<input type="hidden" name="input_star" id="'+id+'_input" projectid="'+id+'" sumNum="3"/>'+
...
function queryOrganizeList(obj){
	requestPost("/app/getOrganizeByUser",obj,"organize",true,function(resultMap){
		if(resultMap != undefined && resultMap != null){
...
function showOrganizeInfo(list){
	if(list.length > 0 && list != null){
		...
		$.each(list,function(i,map){
			...
			html += '<li '+cls+' name="organize_li" organizeid="'+map.organizeid+'" organizename="'+map.organizename+'" identityname="'+map.identityname+'">'+map.organizename+'</li>';
		});

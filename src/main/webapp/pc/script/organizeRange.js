		...
		for(var i=0;i<list.length;i++){
			if(list[i].parentid=="" || typeof(list[i].parentid)=="undefined"){
            	...
            	"<li id=\"li1_"+list[i].organizeid+"\">"+                    	                    	
                	...
                	"<span class=\"bg_show\" onclick=\"changenextul1(this,'"+list[i].isonlyread+"','"+list[i].organizeid+"')\">"+
					"<i id=\"companyname\">"+list[i].organizename+"</i>";
					if(list[i].isonlyread != 1){
						temp+="<a href=\"javascript:void(0)\" onclick=\"addorganize1(this,'"+list[i].organizeid+"','"+list[i].organizename+"')\" class=\"add\">添加</a>";
					}
					temp+="</span>"+
                "</li>"+
                "</ul>";
	...
	for(var i=0;i<list.length;i++){
		if(list[i].parentid==organizeid){
			...
			if(list[i].childnum>0){
				temp+="<div class=\"gray_line\" style=\"display:none\"></div><span class=\"bg_hidden\" onclick=\"changenextul1(this,'"+list[i].isonlyread+"','"+list[i].organizeid+"')\">";
				
			}else{
				temp+="<span class=\"bg_last\" onclick=\"changenextul1(this,'"+list[i].isonlyread+"','"+list[i].organizeid+"')\">";
			}
...
function addorganize1(obj,organizeid,organizename){
	$('#rangeul').find("input[value="+organizeid+"]").parent().remove();
	var temp="<li><input type=\"hidden\" name=\"organizeid\" value=\""+organizeid+"\" organizename='"+organizename+"'/>"+organizename+"<a class=\"del\"><img src=\"../userbackstage/images/public/del.png\" alt=\"删除\" onclick=\"deleteorganizeuser(this)\" /></a></li>";
	$('#rangeul').append(temp);
}
...
function changenextul1(obj,isonlyread,organizeid){
	if(isonlyread != '1'){
		//回调查询
		callbackfunc(organizeid);
		$('#organizetree1').find("i").attr("class","");

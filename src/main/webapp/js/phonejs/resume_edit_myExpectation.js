...
$(function(){
		...
		$.ajax({
			url:"/interface/initEditResumeTwo",
			data:"userid="+userid+"&resumeid="+resumeid,
			type:"post",
			success:function(resultMap){
				//回显已经添加的信息
				var resumeInfo = resultMap.data.resumeinfo;
				
				if(resumeInfo.expectposition == "" || resumeInfo.expectposition == null){
					resumeInfo.expectposition = "无";
				}
				$("#industryname").text(resumeInfo.expectposition);
				if(resumeInfo.expectcity == "" || resumeInfo.expectcity == null){
					resumeInfo.expectcity = "无";
				}
				$("#cityname").text(resumeInfo.expectcity);
				if(resumeInfo.expectsalary == "" || resumeInfo.expectsalary == null){
					resumeInfo.expectsalary = "无";
				}
				$("#salaryname").text(resumeInfo.expectsalary);
				if(resumeInfo.intention == "" || resumeInfo.intention == null){
					resumeInfo.intention = "";
				}
				$("#intention").text(resumeInfo.intention);
				
				$("#industryid").val(resumeInfo.expectpositionid);
				$("#cityid").val(resumeInfo.expectcityid);
				$("#salaryid").val(resumeInfo.expectsalaryid);
				
				$('#saveArea').attr("cityid",resumeInfo.expectcityid);
				$('#saveArea').attr("cityname",resumeInfo.expectcity);
				$('#saveIndustry').attr("industryid",resumeInfo.expectpositionid);
				$('#saveIndustry').attr("industryname",resumeInfo.expectposition);
				$('#saveSalary').attr("salaryid",resumeInfo.expectsalaryid);
				$('#saveSalary').attr("salaryname",resumeInfo.expectsalary);
...
/*-------------------------------------------------*/
	...
	function saveMyExpect(){
		$.ajax({
			url:"/interface/editResume",
			data:$('#expectForm').serialize(),
			type:"post",
			success:function(map){
				if(map.status == 0){
					var resumeid = $('#resumeid').val();
					location.href="/weixin/jumpPage/getMyResume?resumeid="+resumeid;
				}
			},
			error:function(){}
		});
